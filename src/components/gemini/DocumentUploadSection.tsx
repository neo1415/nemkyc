...
export const DocumentUploadSection: React.FC<DocumentUploadSectionProps> = ({
  ...
  React.useEffect(() => {
    if (currentFile && uploadState.status === 'idle' && !uploadState.file) {
      // Restore the file and show as uploaded (but not necessarily verified)
      setUploadState({
        status: 'idle',
        file: currentFile,
        progress: 0
  ...
  }, [currentFile, uploadState.status, uploadState.file, onStatusChange]);
      ...
      if (result.success) {
        ...
        if (result.verificationResult) {
          try {
            ...
            // If session update fails, log error and show warning to user
            console.error('❌ Form session update failed:', sessionError);
            
            // Show a warning to the user that document was processed but form integration failed
            setUploadState({
              status: 'verified',
              file,
              progress: 100,
              result,
              analysis,
              error: 'Document processed successfully, but there was an issue updating the form. You may need to re-upload the document.'
            });
        ...
        // Integrate with form state - save file for form submission
        onFileSelect?.(file);
  ...
  const getStatusIcon = () => {
    switch (uploadState.status) {
      ...
      case 'verified':
        // Show warning icon if there's an error message (low confidence case)
  ...
  const getBorderColor = () => {
    switch (uploadState.status) {
      case 'verified':
        // Show yellow border only if there are critical mismatches, green for successful verification
        if (uploadState.error) {
          return 'border-yellow-300';
        }
        const hasCriticalIssues = uploadState.result?.verificationResult?.mismatches?.some(m => m.isCritical) || false;
        return hasCriticalIssues ? 'border-yellow-300' : 'border-green-300';
      case 'failed':
