...
export const schema3 = yup.object().shape({
  ...
  employersPhoneNumber2: yup.string().matches(/^[0-9]+$/, 'Employer\'s Phone Number must be numeric').min(11).max(11).transform(sanitizeString),
  phoneNumber2: yup.string().matches(/^[0-9]+$/, 'Phone Number must be numeric').min(11).max(11).transform(sanitizeString),
