...
const AdminSmartMotoristProtectionClaimsTable: React.FC = () => {
  ...
  const fetchForms = async () => {
    try {
      setLoading(true);
      const formsRef = collection(db, 'smart-motorist-protection-claims');
  ...
  const handleView = (id: GridRowId) => {
    navigate(`/admin/form/smart-motorist-protection-claims/${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'smart-motorist-protection-claims', selectedFormId));
  ...
  const exportToCSV = () => {
    ...
    link.download = `smart-motorist-protection-claims-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <Typography variant="h4" component="h1" gutterBottom>
              Smart Motorist Protection Claims Management
            </Typography>
            <Button
              variant="outlined"
              startIcon={<GetApp />}
...
export default AdminSmartMotoristProtectionClaimsTable;