...
const AdminSmartStudentsProtectionClaimsTable: React.FC = () => {
  ...
  const fetchForms = async () => {
    try {
      setLoading(true);
      const formsRef = collection(db, 'smart-students-protection-claims');
  ...
  const handleView = (id: GridRowId) => {
    navigate(`/admin/form/smart-students-protection-claims/${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'smart-students-protection-claims', selectedFormId));
  ...
  const exportToCSV = () => {
    ...
    link.download = `smart-students-protection-claims-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <Typography variant="h4" component="h1" gutterBottom>
              Smart Students Protection Claims Management
            </Typography>
            <Button
              variant="outlined"
              startIcon={<GetApp />}
...
export default AdminSmartStudentsProtectionClaimsTable;