...
const AdminProfessionalIndemnityClaimsTable: React.FC = () => {
  ...
  const [claims, setClaims] = useState<ProfessionalIndemnityClaimData[]>([]);
  ...
  const fetchClaims = async () => {
    try {
      const claimsRef = collection(db, 'professional-indemnity-claims');
      ...
      const claimsData = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as ProfessionalIndemnityClaimData[];
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this claim?')) {
      try {
        await deleteDoc(doc(db, 'professional-indemnity-claims', id));
  ...
  const handleView = (id: string) => {
    navigate(`/admin/claims/professional-indemnity/${id}`);
  };
