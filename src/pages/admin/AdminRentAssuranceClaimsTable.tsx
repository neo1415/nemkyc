...
const AdminRentAssuranceClaimsTable: React.FC = () => {
  const [data, setData] = useState<RentAssuranceClaimData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const querySnapshot = await getDocs(query(collection(db, 'rent-assurance-claims'), orderBy('timestamp', 'desc')));
      
      const fetchedData: RentAssuranceClaimData[] = [];
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this entry?')) {
      try {
        await deleteDoc(doc(db, 'rent-assurance-claims', id));
        await fetchData();
        toast({
          title: 'Success',
  ...
  const handleView = (id: string) => {
    navigate(`/admin/form-viewer/rent-assurance-claims/${id}`);
  };

  const filteredData = data.filter((row) => {
    const matchesSearch = 
      row.policyNumber?.toLowerCase().includes(searchText.toLowerCase()) ||
      row.nameOfInsured?.toLowerCase().includes(searchText.toLowerCase()) ||
      row.email?.toLowerCase().includes(searchText.toLowerCase()) ||
      row.nameOfLandlord?.toLowerCase().includes(searchText.toLowerCase());
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" gutterBottom sx={{ color: '#8B5A3C', fontWeight: 'bold' }}>
        Rent Assurance Claims Management
      </Typography>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3, alignItems: 'center' }}>
        <TextField
          ...
          placeholder="Search by policy, insured, email..."
          sx={{ minWidth: 300 }}
        />
