...
interface IndividualNFIUViewerProps {
  data: any;
  onClose: () => void;
}

const IndividualNFIUViewer: React.FC<IndividualNFIUViewerProps> = ({ data, onClose }) => {
  ...
  const downloadPDF = async () => {
    const element = document.getElementById('individual-nfiu-pdf-content');
    if (!element) return;
    ...
    try {
      ...
      pdf.setFontSize(16);
      pdf.setTextColor(0, 0, 0);
      pdf.text('Individual NFIU Form', 20, 65);
      ...
      pdf.save(`Individual_NFIU_${data.firstName || 'form'}_${data.lastName || ''}.pdf`);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Individual NFIU Form</h2>
          <p className="text-muted-foreground">Form ID: {data.id || 'N/A'}</p>
          <p className="text-muted-foreground">Form Type: Individual NFIU</p>
        </div>
      ...
      <div id="individual-nfiu-pdf-content" className="space-y-6 bg-white p-6">
...
export default IndividualNFIUViewer;