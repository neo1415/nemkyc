...
const AdminFisheryFishFarmClaimsTable: React.FC = () => {
  ...
  const fetchForms = async () => {
    try {
      setLoading(true);
      const formsRef = collection(db, 'fishery-fish-farm-claims');
  ...
  const handleView = (id: GridRowId) => {
    navigate(`/admin/form/fishery-fish-farm-claims/${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'fishery-fish-farm-claims', selectedFormId));
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    try {
      await updateDoc(doc(db, 'fishery-fish-farm-claims', id), {
        status: newStatus
      });
  ...
  const exportToCSV = () => {
    ...
    const headers = [
      ...
      'Date of Loss', 'Type of Loss', 'Date and Period Loss Occurred', 'Cause of Death', 
      'Disease Specification', 'Other Cause Explanation', 'Vet/Fish Expert Details',
      'Claim Estimate', 'Previous Loss Particulars', 'Contact Details at Loss Premises',
    ...
    const rows = filteredForms.map(form => [
      ...
      formatDate(form.dateOfLoss),
      getValue(form, 'typeOfLoss'),
      getValue(form, 'dateAndPeriodLossOccurred'),
      getValue(form, 'causeOfDeath'),
      getValue(form, 'diseaseSpecification'),
      getValue(form, 'otherCauseExplanation'),
      getValue(form, 'vetOrFishExpertDetails'),
    ...
    link.download = `fishery-fish-farm-claims-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 130,
      renderCell: (params: any) => formatDate(params.row.dateOfLoss),
    },
    {
      field: 'typeOfLoss',
      headerName: 'Type of Loss',
      width: 180,
      renderCell: (params: any) => getValue(params.row, 'typeOfLoss'),
    },
    {
      field: 'dateAndPeriodLossOccurred',
      headerName: 'Date & Period Loss Occurred',
      width: 200,
      renderCell: (params: any) => getValue(params.row, 'dateAndPeriodLossOccurred'),
    },
    {
      field: 'causeOfDeath',
    ...
    {
      field: 'vetOrFishExpertDetails',
      headerName: 'Vet/Fish Expert Details',
      width: 200,
      renderCell: (params: any) => getValue(params.row, 'vetOrFishExpertDetails'),
    },
    {
      field: 'claimEstimate',
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <Typography variant="h4" component="h1" gutterBottom>
              Fishery & Fish Farm Claims Management
            </Typography>
            <Button
              variant="outlined"
              startIcon={<GetApp />}
...
export default AdminFisheryFishFarmClaimsTable;
