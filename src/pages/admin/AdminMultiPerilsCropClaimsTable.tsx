...
const AdminMultiPerilsCropClaimsTable: React.FC = () => {
  ...
  const fetchForms = async () => {
    try {
      setLoading(true);
      const formsRef = collection(db, 'multi-perils-crop-claims');
  ...
  const handleView = (id: GridRowId) => {
    navigate(`/admin/form/multi-perils-crop-claims/${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'multi-perils-crop-claims', selectedFormId));
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    try {
      await updateDoc(doc(db, 'multi-perils-crop-claims', id), {
        status: newStatus
      });
  ...
  const exportToCSV = () => {
    const headers = [
      ...
      'Cost of Fertilizer', 'Quantity Salvaged', 'Proportion of Loss', 'Area Affected', 'Claim Estimate', 
      'Estimated Area of Damage', 'Previous Loss Particulars', 'Contact Details at Loss Premises', 'Loss Description',
    ...
    link.download = `multi-perils-crop-claims-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'areaAffected',
      headerName: 'Area Affected',
      width: 150,
      renderCell: (params: any) => getValue(params.row, 'areaAffected'),
    },
    {
      field: 'claimEstimate',
    ...
    },
    {
      ...
      width: 200,
      renderCell: (params: any) => getValue(params.row, 'lossDescription'),
    },
    {
      field: 'status',
      headerName: 'Status',
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <Typography variant="h4" component="h1" gutterBottom>
              Multi-Perils Crop Insurance Claims Management
            </Typography>
            <Button
              variant="outlined"
              startIcon={<GetApp />}
...
export default AdminMultiPerilsCropClaimsTable;
