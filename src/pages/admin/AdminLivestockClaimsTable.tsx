...
const AdminLivestockClaimsTable: React.FC = () => {
  ...
  const fetchForms = async () => {
    try {
      setLoading(true);
      const formsRef = collection(db, 'livestock-claims');
  ...
  const handleView = (id: GridRowId) => {
    navigate(`/admin/form/livestock-claims/${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'livestock-claims', selectedFormId));
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    try {
      await updateDoc(doc(db, 'livestock-claims', id), {
        status: newStatus
      });
  ...
  const exportToCSV = () => {
    ...
    const headers = [
      ...
      'Loss Date From', 'Loss Date To', 'Cause of Death', 'Disease Specification', 'Other Cause Explanation',
      'Vet Surgeon Details', 'Claim Estimate', 'Previous Loss Particulars', 'Contact Details at Loss Premises',
    ...
    link.download = `livestock-claims-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
...
export default AdminLivestockClaimsTable;
