...
const AdminFireSpecialPerilsClaimsTable: React.FC = () => {
  ...
  const [claims, setClaims] = useState<FireSpecialPerilsClaimData[]>([]);
  ...
  const fetchClaims = async () => {
    try {
      const claimsRef = collection(db, 'fireSpecialPerilsClaims');
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this claim?')) {
      try {
        await deleteDoc(doc(db, 'fireSpecialPerilsClaims', id));
  ...
  const handleView = (id: string) => {
    navigate(`/admin/claims/fire-special-perils/${id}`);
  };
  ...
  return (
    <Box sx={{ padding: 3 }}>
      <Paper sx={{ padding: 2, marginBottom: 2 }}>
        ...
        >
          Fire & Special Perils Claims Management
        </Typography>
