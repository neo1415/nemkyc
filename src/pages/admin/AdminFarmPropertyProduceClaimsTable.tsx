...
const AdminFarmPropertyProduceClaimsTable: React.FC = () => {
  ...
  const fetchForms = async () => {
    try {
      setLoading(true);
      const formsRef = collection(db, 'farm-property-produce-claims');
  ...
  const handleView = (id: GridRowId) => {
    navigate(`/admin/form/farm-property-produce-claims/${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'farm-property-produce-claims', selectedFormId));
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    try {
      await updateDoc(doc(db, 'farm-property-produce-claims', id), {
        status: newStatus
      });
  ...
  const formatDamagedItems = (items: any[]): string => {
    if (!items || !Array.isArray(items) || items.length === 0) {
      return 'N/A';
    }
    
    return items.map((item, index) => 
      `Item ${index + 1}: ${item.itemDescription || 'N/A'} (Qty: ${item.numberOrQuantity || 'N/A'}, Value: ₦${item.valueBeforeLoss || 'N/A'})`
    ).join('; ');
  };
  ...
  const exportToCSV = () => {
    ...
    const headers = [
      'ID', 'Created At', 'Policy Number', 'Name of Insured', 'Phone Number', 'Farm Address',
      'Date of Incident', 'Cause of Loss', 'Additional Loss Details',
      'Damaged Items', 'Status', 'Submitted At', 'Submitted By'
    ];
    ...
    const rows = filteredForms.map(form => [
      ...
      getValue(form, 'farmAddress'),
      formatDate(form.dateOfIncident),
      getValue(form, 'causeOfLoss'),
      getValue(form, 'additionalLossDetails'),
      formatDamagedItems(form.damagedItems),
    ...
    link.download = `farm-property-produce-claims-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 200,
      renderCell: (params: any) => getValue(params.row, 'farmAddress'),
    },
    {
      field: 'dateOfIncident',
      headerName: 'Date of Incident',
      width: 130,
      renderCell: (params: any) => formatDate(params.row.dateOfIncident),
    },
    {
      field: 'causeOfLoss',
    ...
    {
      field: 'damagedItems',
      headerName: 'Damaged Items',
      width: 300,
      renderCell: (params: any) => formatDamagedItems(params.row.damagedItems),
    },
    {
      field: 'status',
      headerName: 'Status',
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <Typography variant="h4" component="h1" gutterBottom>
              Farm Property & Produce Claims Management
            </Typography>
            <Button
              variant="outlined"
              startIcon={<GetApp />}
...
export default AdminFarmPropertyProduceClaimsTable;
