...
const AdminSmartArtisanProtectionClaimsTable: React.FC = () => {
  ...
  const fetchForms = async () => {
    try {
      setLoading(true);
      const formsRef = collection(db, 'smart-artisan-protection-claims');
  ...
  const handleView = (id: GridRowId) => {
    navigate(`/admin/form/smart-artisan-protection-claims/${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'smart-artisan-protection-claims', selectedFormId));
  ...
  const exportToCSV = () => {
    ...
    link.download = `smart-artisan-protection-claims-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <Typography variant="h4" component="h1" gutterBottom>
              Smart Artisan Protection Claims Management
            </Typography>
            <Button
              variant="outlined"
              startIcon={<GetApp />}
...
export default AdminSmartArtisanProtectionClaimsTable;