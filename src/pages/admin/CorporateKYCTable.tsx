...
const CorporateKYCTable: React.FC = () => {
  ...
  const exportToCSV = () => {
    ...
    const headers = [
      'ID', 'Created At', 'NEM Branch Office', 'Insured', 'Office Address', 'Ownership of Company',
      'Contact Person', 'Website', 'Incorporation Number', 'Incorporation State', 'Date of Incorporation',
      'CAC Number', 'BVN Number', 'NIN Number', 'Contact Person Mobile', 'Tax ID Number', 'Email Address',
      'Nature of Business', 'Estimated Turnover', 'Premium Payment Source',
      ...
      'Verification Document Type', 'Signature'
    ];
    ...
    const rows = filteredForms.map(form => {
      ...
      const getPremiumSource = () => {
        return shouldShowConditionalField(form, 'premiumPaymentSource', 'premiumPaymentSourceOther') 
      ...
      return [
        ...
        getValue(form, 'branchOffice'), getValue(form, 'insured'), getValue(form, 'officeAddress'),
        getValue(form, 'ownershipOfCompany'), getValue(form, 'contactPerson'), getValue(form, 'website'),
        getValue(form, 'incorporationNumber'), getValue(form, 'incorporationState'),
        formatDate(form.dateOfIncorporationRegistration), getValue(form, 'cacNumber'),
        getValue(form, 'BVNNumber'), getValue(form, 'NINNumber'), getValue(form, 'contactPersonNo'),
        getValue(form, 'taxIDNo'), getValue(form, 'emailAddress'), getValue(form, 'natureOfBusiness'),
        getValue(form, 'estimatedTurnover'), getPremiumSource(),
        ...
        getDir1('employersName'), getDir1('employersPhoneNumber'), getDir1('residentialAddress'),
        getDir1('taxIDNumber'), getDir1('idType'), getDir1('idNumber'), getDir1('issuingBody'),
        getDir1Date('issuedDate'), getDir1Date('expiryDate'), getDir1Income(),
        ...
        getDir2('employersName'), getDir2('employersPhoneNumber'), getDir2('residentialAddress'),
        getDir2('taxIDNumber'), getDir2('idType'), getDir2('idNumber'), getDir2('issuingBody'),
        getDir2Date('issuedDate'), getDir2Date('expiryDate'), getDir2Income(),
        getValue(form, 'companyNameVerificationDoc'), getValue(form, 'signature')
      ];
    });
    ...
    link.download = `corporate-kyc-forms-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
