 ...
 * 
 * Monitors the health of external APIs (Datapro, VerifyData) and tracks system metrics.
 * 
 * Features:
...
async function performHealthCheck() {
  ...
  // Check Datapro API
  await checkDataproHealth(timestamp);
  
  // Check VerifyData API
  await checkVerifydataHealth(timestamp);
}

/**
 * Check Datapro API health
 * @param {Date} timestamp - Timestamp of health check
 */
async function checkDataproHealth(timestamp) {
  try {
    ...
    console.error(`[HealthMonitor] Datapro health check failed: ${error.message}`);
 ...
 * @param {Date} timestamp - Timestamp of health check
 */
async function checkVerifydataHealth(timestamp) {
  try {
    // Check if API is configured
    if (!VERIFYDATA_SECRET_KEY) {
      await saveHealthStatus({
        service: 'verifydata',
        status: 'not_configured',
        message: 'VERIFYDATA_SECRET_KEY not configured',
        timestamp,
        responseTime: null
      });
      return;
    ...
    // Ping the API with a simple request
    const startTime = Date.now();
    const result = await pingVerifydataAPI();
    const responseTime = Date.now() - startTime;
    
    if (result.success) {
      console.log(`[HealthMonitor] VerifyData API is UP (${responseTime}ms)`);
      ...
      console.error(`[HealthMonitor] VerifyData API is DOWN: ${result.error}`);
      ...
      await generateAlert({
        type: 'api_down',
        service: 'verifydata',
        message: `VerifyData API is down: ${result.error}`,
        timestamp,
        severity: 'critical'
      });
    }
  } catch (error) {
    console.error(`[HealthMonitor] VerifyData health check failed: ${error.message}`);
...
/**
 * Ping VerifyData API to check if it's responding
 * @returns {Promise<{success: boolean, error?: string, errorCode?: string}>}
 */
function pingVerifydataAPI() {
  return new Promise((resolve) => {
    // Use a test RC number for health check (this will fail validation but confirms API is responding)
      ...
      res.on('end', () => {
        ...
        if (res.statusCode === 200 || res.statusCode === 400) {
          ...
          try {
            ...
            // If we get a response with statusCode, API is up
            ...
            // If we can't parse but got a response, API is still up
            resolve({ success: true });
          }
        } else if (res.statusCode === 500) {
          resolve({ 
            success: false, 
            error: `API returned server error: ${res.statusCode}`,
 ...
 * @param {string} service - Optional service name ('datapro' or 'verifydata')
 * @returns {Promise<object|null>} Latest health status
 */
async function getHealthStatus(service = null) {
  if (!db) return null;
  ...
  try {
    if (service) {
      ...
      snapshot.docs.forEach(doc => {
        const data = doc.data();
        if (data.service && !statuses[data.service]) {
          statuses[data.service] = data;
        }
      });
 ...
 */
async function getHealthHistory(limit = 100, service = null) {
  if (!db) return [];
  
  try {
    let query = db.collection('api-health-status')
      .orderBy('timestamp', 'desc')
      .limit(limit);
    
    if (service) {
      query = db.collection('api-health-status')
        .where('service', '==', service)
        .orderBy('timestamp', 'desc')
        .limit(limit);
    }
