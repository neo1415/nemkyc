...
describe('Encryption Utility', () => {
  ...
  describe('Key validation', () => {
    ...
    test('should throw error if ENCRYPTION_KEY is not set', () => {
      ...
      // Need to reload module to test this
      jest.resetModules();
      const { encryptData: encryptDataNoKey } = require('../encryption.cjs');
      
      expect(() => encryptDataNoKey('test')).toThrow('ENCRYPTION_KEY environment variable is not set');
    ...
    test('should throw error if ENCRYPTION_KEY is wrong length', () => {
      ...
      jest.resetModules();
      const { encryptData: encryptDataBadKey } = require('../encryption.cjs');
      
      expect(() => encryptDataBadKey('test')).toThrow('ENCRYPTION_KEY must be 32 bytes');
  ...
  describe('generateEncryptionKey', () => {
    ...
    test('generated key should work for encryption', () => {
      ...
      jest.resetModules();
      const { encryptData: encryptWithNewKey, decryptData: decryptWithNewKey } = require('../encryption.cjs');
      
      const plaintext = '12345678901';
      const { encrypted, iv } = encryptWithNewKey(plaintext);
      const decrypted = decryptWithNewKey(encrypted, iv);
